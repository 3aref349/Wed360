...
const createArticle = async (req: Request, res: Response) => {
    const { description, title ,authorName,imageUrn} = req.body;
   ...
   console.log("create aRticle")
        ...
        if (!description) errors.article = "cannot be empty !!";
        if(Object.keys(errors).length>0) throw new Error("cannot be empty !!")
        const newarticle = await new Articles({ description, title,authorName,imageUrn }).save()
...
const updateArticle = async (req: Request, res: Response) => {
    ...
    const { description, title ,authorName} = req.body;
    console.log(title)
    console.log(description)
    if (isEmpty(title)) errors.title = "title cannot be empty";
    if (isEmpty(description)) errors.article = "body cannot be empty";
    if (isEmpty(authorName)) errors.article = "body cannot be empty";
...
router.post("/" , user,auth,createArticle);
router.put("/:id", updateArticle);

router.get("/",user,auth,getArticles);
